...
func (s *SessionAffinitySelector) Pick(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, auths []*Auth) (*Auth, error) {
	...
	if scope, _ := opts.Metadata[cliproxyexecutor.CallerScopeMetadataKey].(string); strings.TrimSpace(scope) != "" {
		primaryID = strings.TrimSpace(scope) + "::" + primaryID
		if fallbackID != "" {
			fallbackID = strings.TrimSpace(scope) + "::" + fallbackID
		}
	}
	now := time.Now()
...
func (s *SessionAffinitySelector) OnResult(res Result) {
	...
	if scope, _ := res.Options.Metadata[cliproxyexecutor.CallerScopeMetadataKey].(string); strings.TrimSpace(scope) != "" {
		primaryID = strings.TrimSpace(scope) + "::" + primaryID
		if fallbackID != "" {
			fallbackID = strings.TrimSpace(scope) + "::" + fallbackID
		}
